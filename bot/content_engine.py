...
def get_translation_uniquification_hint(lang: str = "") -> str:
    ...
    v12.0: Enhanced — emphasizes DETAILED posts, footer awareness, and
    editorial commentary for short news.
    ...
    _common_instructions = (
        ...
        "1. НИКОГДА не копируй фразы из источника дословно — пересказывай СВОИМИ словами\n"
        "2. Измени структуру: начни с другого факта, переупорядочи абзацы\n"
        "3. Добавь СВОЙ экспертный комментарий (что это значит для владельцев BMW, аналогии, контекст)\n"
        "4. КОРОТКАЯ НОВОСТЬ: если в источнике мало фактов — ОБЯЗАТЕЛЬНО добавь редакционный "
        "комментарий (мнение Маши, сравнение с другими моделями/двигателями, исторический контекст, "
        "или мнение 'от редакции'). Короткая новость = Маша должна РАСШИРИТЬ тему.\n"
        "5. ДЛИННАЯ НОВОСТЬ: достаточно качественного пересказа + краткое мнение\n"
        "6. Подпись в конце ОБЯЗАТЕЛЬНА: Автор @asmasha_bot / @bmw_mpower_club / #bmw_mpower_club + 2-4 хештега\n"
        "7. Лимиты: с фото — 1024 символа ВКЛЮЧАЯ подпись (текст ~850-900), без фото — до 4096 (оптимально 600-1500)"
    )
    if lang and lang != "ru":
        lang_names = {
            "en": "английском",
        ...
        return (
            f"ПЕРЕВОД: Исходная новость на {lang_name} — ПЕРЕВЕДИ на русский и УНИКАЛИЗИРУЙ. "
            f"Не делай дословный перевод — перескажи СВОИМИ словами с экспертным BMW-комментарием.\n\n"
