...
- Не придумывай праздники и события дня — не привязывай новость к дате, если это не часть новости
...
# нейтральная (шины), чтобы исключить перетаскивание темы в пост.
LOCAL_EXAMPLE = """ПРИМЕР ФОРМАТА ОТВЕТА (тему бери из новости ниже, пример показывает только структуру):
ЗАГОЛОВОК: Всесезонные шины выиграли у летних в городском ритме
ТЕКСТ: Испытания в городском цикле дали неожиданный итог: всесезонные шины тормозят на мокром асфальте на три метра короче летних. Причина — состав резины, стабильный при перепадах температур, плюс рисунок протектора, рассчитанный на редкие заморозки.

Для города, где скорость редко поднимается выше 80 км/ч, эта разница перевешивает минусы. Вывод простой: для спокойной городской езды всесезонка стала разумной альтернативой второму комплекту.
ВОПРОС: Ездите ли круглый год на одном комплекте шин?
ХЭШТЕГИ: #шины #автосоветы"""
...
LOCAL_TASK_REMINDER = ("ТЕПЕРЬ ТВОЯ ЗАДАЧА: напиши СВОЙ пост по авто-новости из этого задания "
                       "(тема: «{title}»). Пример выше — только шаблон структуры. "
                       "НИКОГДА не копируй текст примера и его тему про шины. "
                       "Все факты бери только из новости.")
...
def quality_gate(parsed: dict, min_body: int = 280, max_body: int = 1150) -> tuple:
    ...
    if "всесезонные шины тормозят" in low or "на одном комплекте" in low:
        return False, "example_copy"
    q_low = (parsed.get("question") or "").lower()
    if "на одном комплекте" in q_low:
        return False, "example_copy"
...
# Стиль канала Маши — @bmw_mpower_club (BMW M-тематика)
POST_STYLE = PostStyle(
    channel="@bmw_mpower_club",
    headline_emoji="🏎️",
    default_question="Как вам, M-энтузиасты? Пишите в комментариях 👇",
    hashtag_map={
        "m3": "#BMW_M3", "m4": "#BMW_M4", "m5": "#BMW_M5", "m8": "#BMW_M8",
        "x5 m": "#BMW_X5M", "x6 m": "#BMW_X6M", "xm": "#BMW_XM",
        "m2": "#BMW_M2", "m340": "#M340i", "cs": "#CSL", "csli": "#CSL",
        "neue klasse": "#NeueKlasse", "нейе классе": "#NeueKlasse",
        "alpina": "#Alpina", "альпина": "#Alpina",
        "motorrad": "#Motorrad", "мотоцикл": "#Motorrad",
        "nurburgring": "#Nurburgring", "нюрбургринг": "#Nurburgring", "нюрбург": "#Nurburgring",
        "s58": "#S58", "s63": "#S63", "b58": "#B58", "s55": "#S55",
        "twinpower": "#TwinPower", "турбо": "#турбо", "двигател": "#двигатели",
        "электромобил": "#электромобили", "ix": "#BMW_i",
        "тюнинг": "#тюнинг", "акрапович": "#Akrapovic", "тормоз": "#тюнинг",
        "гт3": "#GT", "gt3": "#GT", "dtm": "#DTM",
    },
    default_hashtags=["#BMW", "#MPower", "#bmw_mpower_club"],
    footer="Автор @asmasha_bot | @bmw_mpower_club",
)
